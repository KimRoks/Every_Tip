...
class PhotoPickerViewController: UIViewController {
    var onConfirm: (([SelectedPhoto]) -> Void)?
    ...
    private func setupConstraints() {
        ...
        photosCollectionView.snp.makeConstraints {
            $0.top.equalTo(confirmButton.snp.bottom).offset(15)
    ...
    private func confirmButtonTapped() {
        PhotoConverter.convertAssetsToSelectedPhotos(selectedAssets)
            .subscribe(onNext: { selectedPhotos in
                self.onConfirm?(selectedPhotos)
                self.dismiss(animated: true)
            }, onError: { error in
                print("사진 변환 실패: \(error)")
            })
